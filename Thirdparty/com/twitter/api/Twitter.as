		...
		*/
		...
		private function searchHandler(e:Event):void
		{
			...
			for each (entryXml in xml.atom::entry)
			{
				...
				var user : Object = [];
				user.name = entryXml.atom::author.atom::name;
				if (String(user.name).length > 0){
					user.screen_name = String(user.name).split(" ")[0];
//					var screen_name : String = String(user.name).split(" ")[0];
//					if (screen_name != null || screen_name != "")
//					{
//						user.name = screen_name;
//					}
				}
				user.url = entryXml.atom::author.atom::uri; 
				var status:TwitterStatus = new TwitterStatus(entry, new TwitterUser(user));
				results.push(status);
			}
